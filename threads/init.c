...
int main (void)
{
  ...
  uint32_t *begin = ptov(1024 * 1024); // 1 MB reserved for OS
  uint32_t *end = ptov(init_ram_pages * PGSIZE); // End of available memory
  size_t user_pages = (end - begin) / PGSIZE / 2; // Reserve half for kernel
  user_pages = user_pages > user_page_limit ? user_page_limit : user_pages;
  init_frame_table(user_pages); //initialize frame table
